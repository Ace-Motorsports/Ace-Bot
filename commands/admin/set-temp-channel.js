...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			...
			await interaction.reply({ content: `The temporary channel lobby has been set to **${channel.name}**.`, flags: 64 });
		}
		catch (error) {
			console.error('Error setting temp channel:', error);
			await interaction.reply({ content: 'There was an error while setting the temporary channel. Please try again later.', flags: 64 });
		}
	},
};